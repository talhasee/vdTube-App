...
class LikedVideoScreen extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _LikedVideosScreen();
}

class _LikedVideosScreen extends State<LikedVideoScreen> {
  ...
  Future<void> likedVideos() async{
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final fetchedVideos = responseBody['data'][0]['likedVideos'];

        setState(() {
          videos = fetchedVideos; // Append new videos
          //SET TO ONE BECAUSE PAGING NOT IMPLEMENTED RIGHT NOW IN LIKED VIDEOS
          totalPages = 1; // Update total pages
          currentPage =  1; // Next page to fetch

        });
        logger.d('Liked Videos fetched successfully');
      } else {
        logger.d('Error fetching Liked videos: ${response.statusCode}');
      }
    } catch (e) {
      logger.e('Error occurred in Liked videos - $e');
    } finally {
      setState(() {
        isLoading = false;
      });
  ...
  Future<void> _refreshPage() async {
    ...
    await likedVideos(); // Fetch the latest videos
  }
